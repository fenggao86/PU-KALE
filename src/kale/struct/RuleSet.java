...
public class RuleSet {
	...
	private int iNumberOfRules; //规则个数
	public ArrayList<Rule> pRule = null;  //规则集合
	...
	public void loadTriples(Set<String> inferredTriples) throws Exception {
		/*BufferedReader reader = new BufferedReader(new InputStreamReader(
				new FileInputStream(fnInput), "UTF-8")); //读取的是groundings.txt文件*/
		pRule = new ArrayList<Rule>();
		
		for (String line : inferredTriples){
			String[] tokens = StringSplitter.RemoveEmptyEntries(StringSplitter
					.split("\t() ", line));

			//读取的规则类似：A => B ,或者 A => B => C
...
//				System.out.println(iTrdHead+" "+iTrdTail+" "+iTrdRelation);
	...
	public void randomShuffle() {
		...
		for (int iID = 0; iID < iNumberOfRules; iID++) { //对每个规则

...
public class Eval_LinkPrediction {
	public int iNumberOfEntities;   //实体个数
	public int iNumberOfRelations;  //关系个数
	public int iNumberOfFactors;  //向量维度
	...
	public void preprocess(
		...
		MatrixE = new Matrix(iNumberOfEntities, iNumberOfFactors); //创建一个矩阵数组
		MatrixE.load(fnMatrixE);  //读取文件，填充entity数组
		
		MatrixR = new Matrix(iNumberOfRelations, iNumberOfFactors);
		MatrixR.load(fnMatrixR);  //读取文件，填充relation数组
		
		BufferedReader train = new BufferedReader(new InputStreamReader(
				new FileInputStream(fnTrainTriples), "UTF-8")); //训练集
		BufferedReader valid = new BufferedReader(new InputStreamReader(
				new FileInputStream(fnValidTriples), "UTF-8")); //验证集
		BufferedReader test = new BufferedReader(new InputStreamReader(
				new FileInputStream(fnTestTriples), "UTF-8")); //测试集
	...
	public void evaluate(String fnTestTriples) throws Exception {
		...
		while ((line = reader.readLine()) != null) { //读取一行
			...
			for (int iLeftID = 0; iLeftID < iNumberOfEntities; iLeftID++) { //遍历entity
				double dValue = 0.0;
				String negTiple = iLeftID + "\t" + iRelationID + "\t" +iObjectID; //换掉head，生成corrupted的三元组
				//针对一个triple，计算value
				for (int p = 0; p < iNumberOfFactors; p++) {
					dValue -= Math.abs(MatrixE.get(iLeftID, p) + MatrixR.get(iRelationID, p) - MatrixE.get(iObjectID, p));
				}
				//如果生成的triple不是已经存在的正确的triple
				if(!lstTriples.containsKey(negTiple)){
					if (dValue > dTargetValue) { //corrupted的triple的value比目标value大
						iLeftRank_filt++; //rank_filter排名+1
					}
					if (dValue == dTargetValue) { //相等
						iLeftIdentical_filt++; //+1
					}
				}	
				if (dValue > dTargetValue) {
					iLeftRank_raw++; //rank_raw+1
				}
				if (dValue == dTargetValue) {
					iLeftIdentical_raw++;
				}
			...
			dTotalMRR_filt += 1.0/(double)dLeftRank_filt;  //看不懂为啥这样计算？？？
			...
			iFiltList.add(dLeftRank_filt);  //添加每一个原始triple的rank_filter
			...
			iRawList.add(dLeftRank_raw); //添加每一个原始triple的rank_raw
			iCnt++;
		...
		Collections.sort(iFiltList); //对rank_filter排序
		int indx=iFiltList.size()/2;
		if (iFiltList.size()%2==0) { //偶数
			dMedian_filt = (iFiltList.get(indx-1)+iFiltList.get(indx))/2.0; //中位数
		}
		else {
			dMedian_filt = iFiltList.get(indx);
		}
		
		Collections.sort(iRawList);  //对rank_raw排序
...
//		String fnMatrixR = "MatrixR.real.best";
		String fnMatrixE = "MatrixE-k50-d0.2-ge0.1-gr0.1-w0.1.best";
		String fnMatrixR = "MatrixR-k50-d0.2-ge0.1-gr0.1-w0.1.best";
